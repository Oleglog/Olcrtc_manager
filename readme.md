...
- QR подписок: один QR передаёт Android-клиенту постоянный subscription URL и optional encrypted mirror metadata. После сканирования клиент сразу запрашивает актуальный состав; сами подключения в QR больше не дублируются.
...
С `server-v1.9.49` кнопка **Состав** показывает все уже подключённые инстансы и позволяет одним сохранением добавить или убрать несколько записей. Там же можно добавить несколько ручных `olcrtc://` URI — по одному на строку.

С `server-v1.9.50` удаление подписки также удаляет её JSON mirror с Yandex Disk. Если Yandex временно недоступен, локальная подписка сохраняется, чтобы очистку можно было повторить без осиротевшего публичного файла.

С `server-v1.9.51` удаление инстанса из Admin UI автоматически удаляет все связанные с ним записи из подписок и запускает обновление их Yandex mirror. Ручные URI при этом не затрагиваются.

С `server-v1.9.52` окончательное удаление инстанса также отключает его `olcrtc-server@N.service`, удаляет instance-specific unit/symlink/drop-in, очищает state directory через systemd и выполняет `daemon-reload`. Кнопки обычной остановки и запуска этого не делают, поэтому выключенные инстансы, которые остаются в панели, сохраняются без изменений.

Используй **QR подписки** в Admin UI, чтобы передать клиенту постоянный URL и optional encrypted mirror metadata. Android-клиент создаёт subscription-группу и сразу запрашивает актуальные подключения с основного URL; если он недоступен и mirror настроен, используется mirror. Подключения в сам QR не вкладываются, поэтому его размер не растёт вместе с подпиской.

Начиная с `server-v1.9.35` есть экспериментальная поддержка encrypted mirror через Yandex Disk API. Сервер шифрует список профилей AES-256-GCM, загружает JSON на Яндекс Диск и кладёт `mirror_url` + `mirror_key` в QR-бандл. Это безопасно для публичного файла, потому что без ключа mirror не расшифровывается. Практическое ограничение: Яндекс Диск может отдавать файл через временные `*.storage.yandex.net` URL, которые у некоторых операторов недоступны. В таком случае mirror не поможет; нужен доступ к основному URL через туннель либо прямой CDN/Object Storage mirror.
...
Если фолбэк не работает — проверь, что свежий QR содержит `m:[...]` и непустой `mk`. В `server-v1.9.49` исправлен таймаут повторной синхронизации, из-за которого при рабочей записи на Yandex Disk QR иногда всё равно получал `m:[]` и `mk:""`.
