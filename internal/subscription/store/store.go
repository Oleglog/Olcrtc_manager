...
func (s *Store) AddInstanceWithSource(slug, rawURI string, sourceInstanceID *int) (*model.Instance, error) {
	if !strings.HasPrefix(rawURI, "olcrtc://") && !strings.HasPrefix(rawURI, "olconnect://") && !strings.HasPrefix(rawURI, "openflux://") {
...
func (s *Store) RefreshLinkedInstances(uris map[int]string) ([]string, error) {
	...
	for sourceID, rawURI := range uris {
		if sourceID < 0 || (!strings.HasPrefix(rawURI, "olcrtc://") && !strings.HasPrefix(rawURI, "olconnect://") && !strings.HasPrefix(rawURI, "openflux://")) {
			continue
		}
...
func ensureCoreParam(rawURI string) string {
	if !strings.HasPrefix(rawURI, "olcrtc://") && !strings.HasPrefix(rawURI, "olconnect://") {
		return rawURI
	}
