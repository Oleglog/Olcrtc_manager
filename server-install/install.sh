...
echo "[*] Installing binary to /usr/local/bin/olcrtc..."
...
# Initial env file. Use room ID = "any" for wb_stream/jazz to trigger
# server-side auto-generation on first start. Preserve an existing pinned
# room ID unless --regenerate was passed.
EXISTING_ROOM=""
if [ -f "$ENV_FILE" ] && [ "$REGENERATE_ROOM" -eq 0 ]; then
...
cat > "$ENV_FILE" <<EOF
# Managed by /usr/local/share/olcrtc/install.sh — re-running the installer is safe.
