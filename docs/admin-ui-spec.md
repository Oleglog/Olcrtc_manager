...
3. Или (v2): автоматически определяет nginx stream config и добавляет
   upstream + SNI запись сам. При этом **автоматически проверяется**
   занятость внутреннего порта upstream на `127.0.0.1`, чтобы не
   создать конфликт с xray Reality target (см. ниже).

### Предупреждение о конфликте портов с 3x-ui / xray

При ручной или автоматической настройке nginx SNI multiplexer внутренний
upstream-порт (тот, на котором слушает HTTP-блок за `stream {}`) **обязан**
быть свободен на `127.0.0.1`.

Типичная ловушка: 3x-ui / xray Reality часто использует порт **9443**
в качестве `target` (поддельного сайта для камуфляжа, параметр `dest`
в `realitySettings`). Если назначить upstream `olcrtc`/`olcrtc-admin`
на тот же `127.0.0.1:9443`, Reality ломается:

- xray подключается к `127.0.0.1:9443` и отправляет обычный HTTP/TLS
  (без `PROXY protocol`);
- nginx-HTTP-блок olcrtc, в свою очередь, может ожидать
  `proxy_protocol` или отвечать совсем другим контентом;
- в итоге клиент получает `SSL_ERROR_SYSCALL` или `400 Bad Request`,
  а туннель Reality перестаёт работать.

**Требование к реализации:**

- Автоконфигурация (v2) перед созданием upstream **должна** проверять
  занятость порта на `127.0.0.1` (например, через `net.Listen` или
  анализ `ss -tlnp`).
- Если порт `9443` занят (или любой другой выбранный по умолчанию),
  система автоматически выбирает следующий свободный (например,
  `10443`, `11443`, `12443` и т.д.).
- В инструкции для ручной настройки (README) порт 9443 **не должен**
  использоваться как жёсткий дефолт для olcrtc; вместо этого
  указывается проверка `ss -tlnp | grep 9443` и выбор альтернативы.
- `proxy_protocol` в `stream {}` и в HTTP-блоке подписок должен
  включаться **только** если xray конфигурирован с
  `"acceptProxyProtocol": true`. В противном случае `proxy_protocol`
  отключается, чтобы не ломать Reality.
